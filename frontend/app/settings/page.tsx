...
function EmailHistoricalBackfillSection({
  ...
  React.useEffect(() => {
    ...
    const nextUser = sessionUser as UserProfile;
    setMe(nextUser);
    setSelectedUserId(prev => prev ?? nextUser.id);
    if (nextUser.role === 'owner' || nextUser.role === 'super_admin') {
...
function ProfileSection() {
  const { user: sessionUser, refresh } = useAuthSession();
  const [user, setUser] = React.useState<UserProfile | null>(() => sessionUser as UserProfile | null);
  const [draft, setDraft] = React.useState<Partial<UserProfile>>({});
  const [loading, setLoading] = React.useState(!sessionUser);
  const [saving, setSaving] = React.useState(false);
